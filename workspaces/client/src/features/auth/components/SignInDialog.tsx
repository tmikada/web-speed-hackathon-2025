...
export const SignInDialog = ({ isOpen, onClose, onOpenSignUp }: Props) => {
  ...
  const validateField = (name: keyof SignInFormValues, value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (e instanceof BetterFetchError && e.status === 401) {
        setSubmitError('アカウントが存在しないか入力した情報が間違っています');
      } else {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    validateField(name as keyof SignInFormValues, value);
  };
